...
mongo.connect(url, (err, client) => {
  ...
  const db = client.db('mailboxD');
  const collection1 = db.collection('round2A2');
  const collection2 = db.collection('round2B2');
  const collection3 = db.collection('round2C2');
  const collection4 = db.collection('round2D2');
  const collection5 = db.collection('round2E2');
  const collection6 = db.collection('round2F2');
...
app.post('/verify', async function(req, res) {
	...
	if (req.body.data.status == "init1-2") {
		...
		sendPackages({status: "init2-3b", round: globalCounter}, "fin2-3b", "http://localhost:8082/deposit1",
	"http://localhost:8082/verify", shuffledObjArray, true);
		...
		unshuffle().then(function(){
			sendPackages("init2-1", "fin2-1", "http://localhost:8083/receive",
	"http://localhost:8083/verify", objectArray, false).then(
	function() {
		reset();
		listening = false;
		res.send("Closed");
